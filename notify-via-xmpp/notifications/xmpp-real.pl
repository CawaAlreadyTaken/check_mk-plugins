...
sub plain_substitute_context {
    my ($string, $pRef, $url_prefix, $debug) = @_;
    warn "D-plain_substitute_context: substitute the following string: '" . $string . "'" if $debug;

    # Sostituisco solo le variabili $KEY$ con i valori in %p
    foreach my $key (keys %{$pRef}) {
        my $val = defined $pRef->{$key} ? $pRef->{$key} : '';
        $string =~ s/\$$key\$/$val/gm;
    }

    # Converte le sequenze letterali "\n" in newline reali (opzionale ma utile)
    $string =~ s/\\n/\n/g;

    warn "D-plain_substitute_context: to: '" . $string . "'" if $debug;
    return $string;
}
...
sub construct_message{
	...
	if ($pRef->{WHAT} eq "SERVICE"){
		$message .= plain_substitute_context($pRef->{PARAMETER_SERVICE_MESSAGE}, $pRef, $url_prefix, 1, $debug);
	}else{
		$message .= plain_substitute_context($pRef->{PARAMETER_HOST_MESSAGE}, $pRef, $url_prefix, 1, $debug);
	}
...
sub xmpp_send {
    ...
    if ($rcpt_is_room) {
        my $join_xml =
            "<presence to='$rcpt/$user'>" .
                "<x xmlns='http://jabber.org/protocol/muc'/>" .
            "</presence>";

        my $res = $cnx->SendXML($join_xml);
        xmpp_check_result('Send(join)', $res, $cnx, $debug);
        warn "D-xmpp_send: sent MUC join presence to room $rcpt as nick $user" if $debug;

        eval { $cnx->Process(1); };
        warn "D-xmpp_send: after Process(1) for room join" if $debug;
    }

    my $body = xmpp_escape($msg);
    my $type = $rcpt_is_room ? 'groupchat' : 'chat';

    my $packet =
        "<message to='$rcpt' type='$type'>" .
            "<body>$body</body>" .
        "</message>";
    ...
    if ($rcpt_is_room) {
        my $leave_xml =
            "<presence to='$rcpt/$user' type='unavailable'>" .
                "<x xmlns='http://jabber.org/protocol/muc'/>" .
            "</presence>";

        $cnx->SendXML($leave_xml);
        warn "D-xmpp_send: sent MUC leave presence to room $rcpt as nick $user" if $debug;
    }
}
...
sub main {
    ...
    $p{PARAMETER_XMPPSERVER} ||= 'changeme.org';

    $p{PARAMETER_USER}       ||= 'alerts';

    # XMPP User password
    $p{PARAMETER_PASSWORD}   ||= 'changeme';

    # 'TLS' or 'SSL'
    $p{PARAMETER_SECURITY}   ||= 'TLS';

    $p{PARAMETER_RESOURCE}   ||= 'checkmk';

    if (defined $p{CONTACT_XMPP} && $p{CONTACT_XMPP} =~ /\@conference\./) {
        $p{PARAMETER_CHATROOM} = 1;
    } else {
        $p{PARAMETER_CHATROOM} = 0;
    }

    # Logs are in ~/var/log/notify.log
    $p{PARAMETER_DEBUG}      ||= 0;

    $p{PARAMETER_SERVICE_MESSAGE} ||= 'Service $SERVICEDESC$ on $HOSTNAME$ is $SERVICESHORTSTATE$: $SERVICEOUTPUT$';
    $p{PARAMETER_HOST_MESSAGE}    ||= 'Host $HOSTNAME$ is $HOSTSHORTSTATE$: $HOSTOUTPUT$';
    ...
    #my $message = "<body>test message</body>";
...
Daniele Cabassi - daniele.cabassi01@gmail.com
